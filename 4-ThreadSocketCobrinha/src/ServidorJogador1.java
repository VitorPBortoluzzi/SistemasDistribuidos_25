...
public class ServidorJogador1 extends javax.swing.JFrame {
     ...
     */
    private void enviaAtualizacaoJogo() throws IOException {
    // 1. Reposiciona a fruta (sempre o Servidor decide)
    Movimenta.posicionaAleatorio(jButtonFruta, jPanelJogador1.getBounds().width,jPanelJogador1.getBounds().height);
        
    // 2. Envia a atualização (Fruta + Placar) para o Cliente
    saida.flush();
    // Usa o construtor de PLACAR, que carrega as coordenadas da fruta e os placares
    Componente cJogo = new Componente(
        jButtonFruta.getBounds().x,
        jButtonFruta.getBounds().y,
        placarJ1,
        placarJ2
    );
    saida.writeObject(cJogo);
}
                ...
                try {
                    ...
                    while (true) {
                        ...
                        if (c.tipo == Componente.JOGADOR) {
                            ...
                            // Cliente J2 reportou que pegou a fruta! (idJogador = 2)
                            placarJ2++;
                            jLabel_placar2.setText("J2: " + placarJ2);
                            
                            // Unificar: Usa o método auxiliar para reposicionar a fruta e enviar a atualização
                            enviaAtualizacaoJogo();
                        }
                    }
                } catch (Exception e) {
    ...
    private void initComponents() {
        ...
        jLabel_placar2.setText("jLabel2");
    ...
    private void jButtonFrutaKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jButtonFrutaKeyPressed
        ...
        if (Movimenta.pegou(jButtonJogador1, jButtonFruta)) {
            placarJ1++;
            jLabel_placar1.setText("J1: " + placarJ1);
            try {
            // Unificar: Usa o método auxiliar para reposicionar a fruta e enviar a atualização
                enviaAtualizacaoJogo(); 
            } catch (Exception e) {
            System.out.println("Erro ao enviar Placar e Fruta para o Cliente!!");
        }
        }
        // Envio da posição J1 para Cliente J2
            try {
                saida.flush();
